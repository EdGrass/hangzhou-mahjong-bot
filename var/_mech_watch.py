...
V_MECH_MIN_ROUNDS = 3200  # ★ R1521：**约 40 房**以下不判，免得把噪声当成 B3。
#   ⚠ 修正 R1481 的单位错：当时写“320（约 40 房，实测 8 局/房）”，
#   而 `_seat_h2h` 的 `rounds` 实测是 **80 局/房**（役3 窗口：8 房 = 640 局）
#   ⇒ 320 局只等于约 **4 房**，机制闸会提前 10 倍点火（实测当时 V 的爆头差仅 ~1.8σ 就会被判 WARN）。
#   40 房 × 80 局/房 = 3200 局 = 原意图。
