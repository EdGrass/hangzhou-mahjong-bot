...
def main(argv=None):
    ...
    for suf, arm in ((a.suffix_a, a.arm_a), (a.suffix_b, a.arm_b)):
        ...
        #   **记录下来并保留为正式赛备选臂**（与 §V.66 选臂口径并行比较）。★ R1557c：
        #   “备选”只意味着**可比较**，不意味着可部署（见下方标记正文里的声明）。
        ...
        try:
            with io.open(B2_OUT, "w", encoding="utf-8", newline="") as f:
                ...
                f.write("注：他们**没有**通过预登记判词 ⇒ 按 §V.161 规则 1 **不进自动候选池**；\n"
                        ...
                        # ★ R1557c：把“只能比较、不能部署”写进**标记正文** —— 第 456 行起的正文会被心跳 0d 原样贴给人。
                        "★ 注意：这只是**比较材料** —— 按 §V.161 A.1（写死“没过阈值的不进候选池”、“不能事后挑”）"
                        "与 campaign8 §4-4，**未判正的臂不入最终臂候选池**，**不得据此写 `var/.final_arm.txt`**。\n")
