...
def to_view(rec):
    ...
    return {
        ...
        # ★ R1184 修：真机视图由 bot/game.py 注入 river / river_len（game.py:553/561），
        #   而本工具早先**没有**这两个字段 ⇒ 所有依赖弃牌河的门控（弃胡河长轴、可见张数、
        #   speedc144/151c/158…）在离线里都跑在"空河"上 ⇒ **足迹不可信**（会假零或假窄）。
        #   现在与真机口径对齐：river = 本局已打出的牌序列，river_len = 其长度。
        "river": list(rec.get("r") or []),
        "river_len": len(rec.get("r") or []),
    }
