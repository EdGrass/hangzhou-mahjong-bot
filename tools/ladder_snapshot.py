...
def main():
    ...
    new = []
    live = []                     # ★ R1272：本次**实时**拉到的快照（--show 也要用，别只打印历史行）
    ...
    # ★ R1272：先打印**实时**值（旧版 --show 只打印历史末行 ⇒ 会把几小时前的数字当前值报出去）
    for r in live:
        print("  [%s] (live) rank=%s rooms=%s score=%s（%.1f 分/房）firsts=%s" % (
            r.get("period"), r.get("rank"), r.get("rooms"), r.get("score"),
            (r.get("score_per_room") or 0), r.get("firsts")))
    for p in periods:
        v, tr = trend(rows, p)
        if not v:
            continue
