...
class TestPure(unittest.TestCase):
    ...
    def test_arm_adopted_newest_file_decides(self):
        with tempfile.TemporaryDirectory() as d:
            old = vfile(d, "_verdict_役3.txt", M + "ADOPT speedvaluebc\n", 1000.0)
            new = vfile(d, "_verdict_役4.txt", M + "REFUSE speedvaluebc\n", 2000.0)
            self.assertIsNone(C.arm_adopted("speedvaluebc", C.load_verdicts([old, new])))
            io.open(new, "w", encoding="utf-8").write(M + "ADOPT speedvaluebc\n")
            self.assertIsNotNone(C.arm_adopted("speedvaluebc", C.load_verdicts([old, new])))

    def test_load_verdicts_skips_bak_and_watch(self):
        with tempfile.TemporaryDirectory() as d:
            vfile(d, "_verdict_watch.py", M + "ADOPT speedvalue\n", 1000.0)
            vfile(d, "_verdict_役2.txt.bak_20260101", M + "ADOPT speedvalue\n", 1000.0)
            good = vfile(d, "_verdict_役2.txt", M + "ADOPT speedvalue\n", 1000.0)
    ...
    def test_decide_prefers_adopted_layer(self):
        with tempfile.TemporaryDirectory() as d:
            p = vfile(d, "_verdict_役3.txt", M + "ADOPT speedvaluebc\n", 1000.0)
    ...
    def test_decide_falls_back_to_baseline(self):
        with tempfile.TemporaryDirectory() as d:
            p = vfile(d, "_verdict_役3.txt", M + "REFUSE speedvaluebc\n", 1000.0)
            arm, elig, chosen = C.decide("speedvalue", ["speedvaluebc"], C.load_verdicts([p]))
    ...
    def test_decide_multiple_adopted_newest_wins(self):
        with tempfile.TemporaryDirectory() as d:
            p3 = vfile(d, "_verdict_役3.txt", M + "ADOPT speedvaluebc\n", 1000.0)
            p5 = vfile(d, "_verdict_役5.txt", M + "ADOPT speedvaluebcvmeld\n", 3000.0)
    ...
    def test_decide_ignores_undecided(self):
        with tempfile.TemporaryDirectory() as d:
            p = vfile(d, "_verdict_役3.txt", M + "UNDECIDED（和牌率 z=+0.9）\n", 1000.0)
...
class TestMain(unittest.TestCase):
    ...
    def test_go_writes_adopted_candidate(self):
        v = vfile(self.d, "_verdict_役3.txt", M + "ADOPT speedvaluebc\n", 1000.0)
