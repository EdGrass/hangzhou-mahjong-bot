...
> ★ **R1448/R1569：下面这三步已由计划任务自动执行**：`HangzhouMajFinalSubmit` @ **10/8 10:00**，
> 若瞬时失败则 `HangzhouMajFinalSubmitRetry` @ **10/8 11:00** 重试（幂等）；失败会写
> `var/.FINAL_NOT_READY`（**不再静默**，心跳 0g 会转述）。下面的手动步骤保留作为**备用/重现**。
> ★ 人工剩两项：10/10 的令牌文件；与**本卡 §3 的易网提交**（10/8 12:00 前）。
...
3. **源码**：优先用**仓库链接**；若要附件则压包含 `.git` 的目录（★ R1589 实测：本仓 `.git` **~59MB**（主要是计划/日志的历史版本），而判官 checkout 只有 **~9MB**（四个权重合计 **0.77MB**）⇒ 附件路线会超线，**请用仓库链接**）
