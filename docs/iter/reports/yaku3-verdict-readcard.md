...
### 0d. ★ R1560 追加投影（2026-09-26 11:2x · **55 房**）：**强手房层**（正式赛对手密度）里两个候选都**明显为负**

> 来源：`python -X utf8 var/_verdict_by_elite.py --since "2026-09-25 20:52:39" --arms speedvalue,speedvaluebc,speedvaluebaotouv5`（只读台账）。
> **只为 9/28 对齐预期，不是判词**；层内房数很少（§V.268）⇒ **不能据此翻转**，但**必须在破平时当原值读**。

| 臂 | 分层 | 房 | 分/房 | 第1率 |
|---|---|---|---|---|
| `speedvalue`（基线） | 全部 | 19 | −13.9 | 15.8% |
| `speedvalue` | **强手房** | 11 | **−23.0** | **18.2%** |
| `speedvaluebc` | 全部 | 19 | **+28.1** | **26.3%** |
| `speedvaluebc` | **强手房** | 11 | **−66.4** | **9.1%** |
| `speedvaluebc` | 弱房 | 8 | +157.9 | 50.0% |
| `speedvaluebaotouv5` | 全部 | 18 | +40.6 | 27.8% |
| `speedvaluebaotouv5` | **强手房** | 11 | **−70.8** | 27.3% |
| `speedvaluebc` | 强手房**≥2** | 5 | −62.2 | 0.0% |
| `speedvaluebaotouv5` | 强手房**≥2** | 4 | −117.8 | 0.0% |

**怎么读（三条）**：

1. **全部列与强手房列方向相反**：两个候选在全部房里都是正的（bc +28.1、V +40.6），而在**强手房**里都比基线差很多（bc −66.4 vs −23.0；V −70.8 vs −23.0）。正式赛对应的正是强手房列 ⇒ **这是判词时最该先看的一列**。
2. **两层方向一致**（不同于役2 的“两层反向”先例）：≥2 层也都是负的 ⇒ 不能用“两层不一致”来解释。
3. **但房数太少**（强手房 11/臂、≥2 层 4–5/臂）⇒ 按 **§V.268** 这只能当“方向读数”，**不能据此把候选判死**；但它与§0c 的机制读数（bc = 赢得更大、V = 胡率更高）并读，都指向**同一个预期：役3 很可能落 NONE 行**。

⇒ **对本役3 的直接含义**：若判词是 `BOXED`（到盒未决定）而需走 §V.66 破平，第一步（强手房分/房）就指向“保持现状”；这与 **§V.281**（役5 槽位是否用来起 `speedvaluerank`）是同一个决定的两面 —— 前者决定“役3 用不用候选”，后者决定“空出来的槽位给谁”。
...
★ **R1561（**9/28 当天必读**）：上表最后一行的“按破平序列收口”**不等于“BOXED 也能判正”**。

- 按 **§V.161 A.1**（写死“没过阈值的不进候选池”“不能事后挑”）与 **§V.279（R1557）**，
  **未判正（含 `BOXED`/`UNDECIDED`、以及 B2）不得进最终候选池**；§V.66/§V.67 的破平序列**只在已判正的臂之间选**
  （它自己的步骤 4/5 也写着“不赌未验证组合臂”）。
- **机器侧的实际行为**：`var/_verdict_watch.py` 在到盒时写的验证文件里，最后一条 `★ 判定：` 仍是 `_gate2` 的 `UNDECIDED(…)`
  （它自己的提示行也写着“采用门槛（主+副 z>=1.50）**不变**”）；而 `_adopt_pair.classify_candidate` 只认 `ADOPT` 开头
  ⇒ **`BOXED` 就是 `adopted=False`** ⇒ 四格落 **NONE 行** ⇒ `HangzhouMajAdoptPairWatch`（**每 10 分钟**）直接起役 4
  （`speedvalue` + `speedvaluemeldp45`）。**没有“等人工破平”的暂停窗口**——唯一例外是
  `var/.VERDICT_RULE_CONFLICT` 那一格（V 主 z≥1.50 但 0<副 z<1.50），那时机器**原地不动**、等人定性。
- **红线（必守）**：**绝不要手改 `var/_verdict_役3*.txt`**（不手搓状态文件）。手写一行 `ADOPT` 会**静默把四格翻成 A/B 行**，
  让一根**主端点从未通过**的臂进到 10/7 的部署链上。
- 若你认为某候选值得保留：它会落 `var/.B2_CANDIDATES`（判词终态、机制成立、主端点未证实）
  ⇒ 10/5 提案会把它作为**旁证**并列比较；那与 **§V.281**（空出来的役位给谁）是**两个不同的决定**。
