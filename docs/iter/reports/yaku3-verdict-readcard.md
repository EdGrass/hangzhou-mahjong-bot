...
**★ R1557 订正**："全部臂/不受该行限制"指的是**列出与比较**的范围；
按 **§V.161 A.1**（写死"没过阈值的不进候选池"、"不能事后挑"）与 **campaign8 §4-4**，
**未判正的臂（包括 B2）不入最终臂候选池** —— 它只能作**旁证**，**不得据此写 `var/.final_arm.txt`**；
人只能在**已判正的臂**之间换行/选臂。（详见计划 §V.279）
