...
| `.B2_CANDIDATES` | `_adopt_pair.py` | **`_final_pick_proposal.b2_section()`**（R1535：把它按**自己那一役的窗口**跑 `_pick_arm` 追加进 10/5 提案）+ 心跳 **0d** | **0d** | 机制成立/主端点未证实 ⇒ 供人并行比较（**不进自动池**，§V.161 规则 4 不变） |
