...
## 默认评估口径（记录于 2026-09-06）

- L1 同桌面：候选×2 vs SpeedE×2，rounds=8，`sim.WALL_RESERVE` 待 C003 落定为 60 前沿用
  引擎默认（=20 但 sim 内长局口径）；首批候选在统一口径下互比即可。
- 判据：N > (2/δ)²×100（单局 σ≈10）；晋级 δ_min=2.0/场；CI 含 0 即 DRAW。
- A/B 命令：`python -m arena.runner --combo "speedxNx2+speedEx2" --games 200 --rounds 8 --seed S`
  （≥2 批不同 seed，合计 ≥400 局）→ `python tools/ab_gate.py --combo ...`。

## 队列

| id | 来源 | 假设（一句话） | 域 | 状态 | 备注 |
|---|---|---|---|---|---|
| C001 | PROJECT.md §7.1 / 复盘差异#2 | tie 内弃刚摸优先（TOP 打新张 vs 我们留新张） | strategy | in-l1 | speedx1，本轮 |
| C002 | PROJECT.md §7.5 | 已见牌扣减重试（SpeedG 思路，曾就绪被清） | strategy | queued | 先 grep 历史 commit 3c269fd |
| C003 | PROJECT.md §7.4 | 短局口径入默认评估（WALL_RESERVE=60） | eval | queued | 基建，影响后续所有判定 |
