...
| C008 | 指南 §1.2 弃胡/财飘设计（P4 线） | 爆头态摸白弃胡打白飘：每飘链 ×2（手留白+飘出=4 再 ×2），弃刚摸白恒回爆头态、每飘只赌一个对手回合圈 | strategy | in-l1 | **speedp 已实现**（run_bot/arena opt-in speedp；门控 max_chain=3/late_river=55 可配）；引擎-服务器 fan 全对齐；sim 端到端 0 违规；本地 sim A/B 信息性跑分后台；真机须平衡设计+oracle 执行验证 |
