...
> 2026-09-08 v24 适配修订：快照语义已从 v14 演进——【快照自带 melds（四家副露
> 数组 {kind,tiles}）与 drawn_tile（本人刚摸）】，下述旧条目 4 的"真机快照无
> melds 靠本地累计"已过时；game.py 以服务器 melds[seat] 为副露权威源，本地
> tracker 仅作字段缺失兜底（自动房竞态下 tracker 分叉曾致 guard 风暴 300-500
> 次/场，已修——见 §5 事故 11）。

1. 快照 `my_hand` 恒为**不含刚摸牌**的 13-3e-g 张；本人摸牌只经
   `tile_drawn` 事件（tile 仅对自己可见，他人恒空）；game.py 按
   `expect=14-3e-g` 长度判断补第 14 张（副露后服务器部分路径已含摸）。
   ⚠ v24 实测：摸后快照可能已含刚摸且 drawn_tile 同给——补牌逻辑按长度
   自适应（len==expect 直接用 / len==expect-1 才补），两种口径均兼容。
...
**正式赛窗口 A/B 终裁 converged**（W1：318 场 δ+0.27/场 CI 含 0（150 场快照
+0.47 后继续衰减，2026-09-08 复核），150 局 0 杠触发）→
不晋级默认；策略实验分支移除（协议机器与审计工具链保留，见
docs/iter/reports/W1-speedh-window.md）。
SpeedK/SpeedU（K1 有效牌 + K2 副露，2026-09-08）：K1/K2 族真机快筛**无大效应**
（uA n18 δ−1.39 CI 含 0，判负即止；w2 复核 n21 δ−4.71——原记 −6.8@12 有误）——
ukeire 类代理与財神自摸竞速脱钩，该族封存（SpeedU-verdict.md；⚠ 会话座位组合
效应 ±5-11/场未控，负向结论待平衡设计复核，见 health-audit-2026-09-08.md）。
微调类候选判定改为 n≈20-25 大效应判据。
SpeedP（打点/弃胡财飘，2026-09-08）：E+爆头摸白弃白飘链 v0 实现并经 sim
端到端验证（双飘 fan8 0 违规）；**频率裁决 paused**——sim 240 巡 0 触发、
e4a 真机 36 胡 0 爆头胡（爆头胡实盘占比 ≤3%）：飘入口频率钉死，代码 opt-in
保留（仅 YouCaiBiKao=true 类配置或白保留策略 v2 下重开，见 queue.md C008）。
...
| 11 | 自动房 claim 后 guard 风暴（v24 快照 melds 未采用，tracker 竞态分叉→每场 300-500 次跳过轮，3s 超时代打） | snap_view 采用服务器 melds[seat] 为副露权威（2026-09-08，实测 guard→0、INVALID→19/场） |
...
## 9. 测量纪律与危机处置记录（2026-09-08，health-audit-2026-09-08.md）

> 背景：六代候选（F/G/x1/x2/K1/K2/river）全部收敛或判负，经系统性复核发现
> 根因不在策略而在【测量体系三层污染 + 行为面频率钉死】：

1. **引擎/服务器口径漂移**：v21（2026-09-07）修订 4白爆头与七对豪华组语义，本地
   引擎/黄金集未跟进 → 已修复并对齐（140 例在线刷新 + preflight v21 探针 +
   tests/test_engine_v21.py；见 c660ccc）。
2. **座位结构污染**：同策略按绝对占位分组 δ 差 ±5-11/场（session_health
   combo_delta 实证）→ 一切真机 A/B 强制【半程换座对或双房反向】+ 位置组合
   分解 + 健康门禁（自迭代方案 §5 R11 修订；tools/ab_balanced.py 合并裁决）。
3. **执行验证缺失** → 全会话自动决策录制（<gid>.dec.jsonl 全局面快照）+
   [xlog] 分歧探针 + oracle 重放审计（tools/oracle_audit.py：E-vs-E 重放 0 分歧
   自检通过）——任何候选事后可证明"真的差异化执行"。
4. **频率钉死**：杠链（SpeedH：150 局 0 触发）、财飘链（speedP：sim 240 巡 0 触发
   + e4a 真机 36 胡 0 爆头胡）——机制均端到端验证正确，但入口频率 <0.5%/局，
   收益上限 <1/场 → 该族（含打点线）paused 登记，除非 YouCaiBiKao=true 类配置。
5. **真机功率硬约束**：同桌 σ_d 实测 8.7-13.9（e4a 13.9）→ δ=2 需 n≈85-190 场
   （1.5-3min/场 ≈ 3-8h）——真机只配测 δ≥3 的大效应候选；小效应一律先过
   离线 oracle（决策重放/进度代理）再谈真机。

**平台收敛性结论（2026-09-08 收官，oracle-v2 + 座位标定双报告）**：SpeedE 在
现行规则（无 YouCaiBiKao、自摸和、财神局）下**已近饱和**——
- 微调族（K1/K2/river/ukeire）：真人对局 1937 决策执行面宽度 ≤5.3%，历史
  收敛/负结果与"真实效应 ≈0 + 房级方差"相容（E×4 标定 T=−0.52 CI 含 0，
  无座名对结构偏倚）；预期 |δ|<1 低于真机可判线（σ 9-14 → n≈150-190），
  平衡复核期望价值为负 → 封存维持；
- 打点/链族（财飘/杠开）：入口频率实测为 0（sim 0/240、e4a 真机 0/36 爆头、
  09-04 真人对局 0/1937 触发）→ 关闭，仅 YouCaiBiKao=true 类规则环境重开；
- 后续投入方向：协议/稳定性域（已 v24 对齐）、正式窗口参赛健康、
  或等新规则环境（有财必拷响/点炮规则）带来新的可判行为面。
