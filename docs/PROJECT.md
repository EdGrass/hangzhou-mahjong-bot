...
1. **tie 内"弃刚摸优先"变体**（复盘差异#2：TOP 玩家打新张 vs 我们留新张）——
   SpeedE 最小改动，Arena 400 局先验；
2. **防守维度**（speed 系全员抢胡无防守）：读取公开弃牌河算危险度、追听风险；
   需要 view 已有 river（game.py 已注入）；
3. **打点 EV 分支**：4 白板/爆头/财飘的高番取舍（规则 ×512 上限已入引擎）；
4. **真机局 ~5-6 巡短局适配**：sim 默认局 ~14 巡偏长——评估时用
   `sim.WALL_RESERVE=60` 校准短局（SpeedF 短局优势实验已证明此口径重要）；
5. **已见牌扣减**（SpeedG 思路，代码曾就绪：waits/进张按 4-手牌-河 修正）；
6. MCTS/搜索型决策（远期）。
