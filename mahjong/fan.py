...
- 分支：平胡 ×1；七对 ×2；豪华七对×N = 2^(N+1)（N = 四张组数：
  实体 4 张各算 1 组；白×4 仅当其余牌无落单（白板两两自配、未消耗于
  补配单张）时算 1 组——v21 修订 2026-09-07；白×4 同时再计 4白板 ×2，叠加）；
- 链命名按 piao：0→杠开（杠上花），1→财飘，2→双财飘，3→三财飘；倍率恒 2^count；
- 4个白板：手牌白 + piao 白 == 4 → ×2；
- 爆头：摸牌前 13 张任意摸皆胡 → ×2（含 4 张白板听任意形，v21 起叠加，见 hu.is_baotou）；
...
def _qidui_groups(counts):
    """七对路径的四张组数：实体 4 张每组 1；白×4 仅在其余实体牌无落单
    （白板全部两两自配、未消耗于补配单张）时算 1 组（v21，2026-09-07）。
    其余牌存在落单（白板作百搭补配成对）时，白×4 不再计豪华组。"""
    n = sum(1 for c in counts[:33] if c == 4)
    singles = sum(1 for i in range(33) if counts[i] % 2 == 1)
    if counts[33] == 4 and singles == 0:
        n += 1
    return n
